<template>
  <div class="talentmain">
    <div class="main_top">
      <div class="main_left">
        <el-input :placeholder="$t('talentPool.search')" v-model="search" @keyup.enter.delete.native="findTalentPoolssearch">
        </el-input>
        <el-button type="success" class="searchbutton green" @click="findTalentPoolssearch" icon="el-icon-search"></el-button>
      </div>
    </div>
      ...
      <div class="tablebox">
        <div class="listbox">
          ...
          <div class="listbutton">
            ...
            <el-button @click="cancelscreenForm('screenForm')" v-if="this.$store.state.Privilege.includes('BUTTON_TALENT_FILTER')" type="info">{{$t('talentPool.screenpeople')}}
              <i class="el-icon-my-shaixuan el-icon--right"></i>
            </el-button>
            <el-button @click="importTalent" v-if="this.$store.state.Privilege.includes('BUTTON_TALENT_EMENDATION')" class="blue">{{$t('talentPool.addtalent')}}
              <i class="el-icon-my-weibiaoti40 el-icon--right"></i>
            </el-button>
        ...
        <!--表格-->
        <el-table ref="multipleTable" :data="talentData.content" tooltip-effect="dark" size="small" row-key="id" style="width: 100%" stripe @row-click='handleRowHandle' @selection-change="selsChange" v-loading="listLoading" element-loading-text="拼命加载中">
          ...
          <el-table-column prop="status" :formatter="posFormat"  :label="$t('news.positonNews.newStatus')" >
          </el-table-column>
    ...
    <el-dialog width="50%" :visible.sync="hotVisible" class="provincedialog" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      ...
      <el-popover v-for="(item,index) in hotCity" :key="index" class="hotcityLi" placement="right-end" trigger="click">
        <el-button slot="reference" @click="findAreaByCityIdhot3(item)">{{item.cityName}}</el-button>
        <el-button @click="showInput(item.cityName,item.cityName,cc)" class="button-new-tag" size="mini">{{item.cityName}}</el-button>
        <el-button v-for="(x,y) in areaDatahot3" :key="y" class="button-new-tag" size="mini" @click="showInput(item.cityName,x.areaName)">{{x.areaName}}</el-button>
      </el-popover>
      <div class="residencetitle">{{$t('talentPool.tip.allp')}}</div>
      <ul class="provinceList">
        <li v-for="(item,index) in allProvince" :key="index" @click="findCityByProvinceId3(item)">{{item.provinceName}}</li>
      </ul>
      <el-popover v-for="(item) in cityData" :key="item.cityName" class="hotcityLi" placement="right-end" trigger="click">
        <el-button slot="reference" @click="findAreaByCityId3(item)">{{item.cityName}}</el-button>
        <el-button @click="showInput(item.cityName,item.cityName,cc)" class="button-new-tag" size="mini">{{item.cityName}}</el-button>
        <el-button v-for="(x,y) in areaData3" :key="y" class="button-new-tag" size="mini" @click="showInput(item.cityName,x.areaName)">{{x.areaName}}</el-button>
      </el-popover>
    </el-dialog>

    <!-- 行业 -->
    <el-dialog width="50%" :visible.sync="screentradeVisible" append-to-body class="tradedialog" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <div class="surebutton">
      ...
      <ul>
        <li v-for="(item,index) in tradeList" :key="index" class="tradeList">
          ...
          <div class="tradesmall">
            <el-button v-for="(v,i) in item.appTrades" :label="v.tradeName" class="button-new-tag" size="mini" :key="i" @click="showInputscreentrade(v.tradeName)">{{v.tradeName}}</el-button>
          </div>
        </li>
      </ul>
    </el-dialog>
    ...
    <!--筛选弹出框-->
    <el-dialog :visible.sync="screenFormVisible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
        ...
        <el-tab-pane :label="$t('recruitSystem.screen.infoScreen')">
          <el-form :model="screenForm">
            <el-form-item :label="$t('talentPool.tip.keyword')" :label-width="formLabelWidth">
              <el-input v-model="screenForm.keyword" auto-complete="off"></el-input>
            </el-form-item>
            ...
            <el-form-item :label="$t('recruitSystem.screen.job')" :label-width="formLabelWidth" :readonly="true">
              <div @click="screenjobVisible=true">
                <el-input v-model="screenForm.job" auto-complete="off">
                  <i slot="suffix" class="el-input__icon el-icon-edit"></i>
                </el-input>
              </div>
            </el-form-item>
            <el-form-item :label="$t('news.positonNews.trade')" :label-width="formLabelWidth" :readonly="true">
              <div @click="screentradeVisible=true">
                <el-input v-model="screenForm.trade" auto-complete="off">
                  <i slot="suffix" class="el-input__icon el-icon-edit"></i>
                </el-input>
              </div>
            </el-form-item>
            <el-form-item :label="$t('recruitSystem.screen.experience')" :label-width="formLabelWidth" class="work">
              <el-input v-model="screenForm.expericenceFrom" auto-complete="off">
              </el-input>
              <div class="line"></div>
              <el-input v-model="screenForm.expericenceTo" auto-complete="off">
              </el-input>
            </el-form-item>
            ...
            <el-form-item :label="$t('news.positonNews.hopeSalary')"  :label-width="formLabelWidth" class="money">
              <el-input v-model="screenForm.hopeSalaryFrom" auto-complete="off">
        <template slot="append">{{$t('recruitSystem.screen.price')}}</template>
              </el-input>
              <div class="line"></div>
              <el-input v-model="screenForm.hopeSalaryTo" auto-complete="off">
             <template slot="append">{{$t('recruitSystem.screen.price')}}</template>
              </el-input>
            </el-form-item>
          </el-form>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="success" class="green" @click="search4ES">{{$t('recruitSystem.screen.screening')}}</el-button>
    ...
    <el-dialog :visible.sync="personnelFormVisible" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false" class="mytalent">
      ...
      <el-dialog width="50%" :visible.sync="jobVisible" append-to-body class="tradedialog" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
            ...
            <ul class="residenceul">
              <li v-for="(item,index) in jobList" :key="index" class="tradeList">
                <div class="tradebig">{{item.jobName|EnjobName}}</div>
                <div class="jobbig">
                  <el-popover v-for="(v,i) in item.appJobs" :key="i" class="jobLi" placement="right-end" trigger="click">
                    <el-button slot="reference">{{v.jobName|EnjobName}}</el-button>
                    <el-button :label="paralang?v.jobNameEn:v.jobName" class="button-new-tag" size="mini" @click="showInputjob(paralang?v.jobNameEn:v.jobName)">{{v.jobName|EnjobName}}</el-button>
                    <el-button v-for="(x,y) in v.appJobs" :label="paralang?x.jobNameEn:x.jobName" :key="y" class="button-new-tag" size="mini" @click="showInputjob(paralang?x.jobNameEn:x.jobName)">{{x.jobName|EnjobName}}</el-button>
                  </el-popover>
                </div>
              </li>
            </ul>
        ...
        <el-tab-pane :label="$t('talentPool.tip.tab01')">
          <el-form :model="baseinfoForm" class="baseinfo" label-width="200px">
            ...
            <el-form-item :label="$t('talentPool.tip.arrivalTime')">
              ...
              <!-- <div class="block">
                <el-date-picker :editable="false" v-model.trim="baseinfoForm.hopeTime" type="date" :placeholder="$t('talentPool.tip.chooseDate')" format="yyyy-MM-dd" value-format="yyyy年MM月dd日" :picker-options="pickerOptionsarrive">
                </el-date-picker>
              </div> -->
          ...
          <div class="savetabbutton">
              <el-button type="success green" style="margin-top:20px" @click="saveTalentPool" :loading="isSave1">{{$t('btn.save')}}</el-button>
...
export default {
  ...
  methods: {
    ...
    findTalentPools() {
      ...
      this.$http.post(api.talent.findTalentPools(), formatUrl(para)).then(
        res => {
          if (res.data.code == "911") {
            location.href = "./login.html";
          }
    ...
    handleSizeChange(val) {
      this.size = val;
      this.findTalentPools();
      // this.toggleSelection(this.$store.state.sels);
    },
    ...
    search4ES() {
      this.$router.push({ path: "poolsearch" });
      this.screenForm.pageIndex = 0;
      this.screenForm.pageSize = 20;
      localStorage.setItem("condition", JSON.stringify(this.screenForm));
      // this.$store.state.screenForm = this.screenForm;
    },
    // multiArea(hotArea, cityArea) {
    //   if (cityArea && typeof cityArea === "object") {
    //     this.locationsArea = cityArea.hasOwnProperty("cityName")
    //       ? cityArea.cityName
    //       : cityArea.areaName;
    //     let currentArea = `${this.locationCity}-${this.locationsArea}`;
    //     this.multiAreaList.push(currentArea);
    //     this.screenForm.residence = Array.from(
    //       new Set(this.multiAreaList)
    //     ).join(",");
    //   }
    // },
    // closeVisible(vm) {
